...
class Hasil_kejuaraan extends CI_Controller {
	...
	function index(){
		$this->input_penilaian();
	}

	function input_penilaian($kdjenjang = null, $kdlomba = null){
		$kode_role = $this->session->userdata('role');
		$kegiatan_object = $this->bm->select_where('tr_kegiatan', 'id', $this->session->userdata('id_kegiatan'))->row();
		if($kegiatan_object->id_jenjang == 3){
			redirect('penilaian/hasil_kejuaraan/input_penegak/'. $kegiatan_object->id_lomba);
		}
		$lomba = $this->bm->select_where('ms_lomba', 'id', $kegiatan_object->id_lomba)->row();;
		// print_r($kegiatan); die;
		$kdjenjang = $kegiatan_object->id_jenjang;
		$kdlomba = $kegiatan_object->id_lomba;
		$nmjenjang = '';
		$peserta = array();
		$data = array(
		...
		$where_juara = array(
			'id_kegiatan' => $kegiatan_object->id,
			'id_jenjang' => $kdjenjang,
			'status' => 1
		);
		$hasil_juara = $this->bm->select_where_array('tr_hasil_kejuaraan', $where_juara)->result();
		...
		$data['kegiatan'] = $kegiatan_object;
		$data['lomba'] = $lomba;
		$data['nmjenjang'] = $nmjenjang;
		$data['jenjang'] = $array_jenjang;
		$data['kejuaraan'] = $kejuaraan;
		$data['hasil_juara'] = $hasil_juara;
		$data['title'] 		= "Input Nilai";
		$data['content'] 	= $this->load->view('admin/penilaian/view_pos_penilaian',$data,true);
		if($kdjenjang == 3){
			$data['content'] 	= $this->load->view('admin/penilaian/view_pos_penilaian_sma',$data,true);
		}
	...
	public function get_peserta($id_kegiatan, $id_jenjang, $id_jenis_regu){
		...
		if($data){
			foreach ($data as $key => $value) {
				$value->peserta = str_pad($value->id, 4, '0', STR_PAD_LEFT) . ' - ' . $this->bm->select_where('ms_sekolah', 'id', $value->id_sekolah)->row()->nmsekolah;
	...
	public function post_juara(){
		...
		if(count(array_unique($juara_putra)) < 5 || count(array_unique($juara_putri)) < 5){
			...
			redirect('penilaian/hasil_kejuaraan');
		}
	...
	public function post_juara_sma(){
		...
		redirect('penilaian/hasil_kejuaraan/input_penegak/'. $kegiatan->id_lomba);
	}
	...
	function input_penegak($kdlomba = null){
		...
		$kegiatan = $this->bm->select_where_array_order('tr_kegiatan', $where, 'id_lomba asc, created_at desc')->row();
		$kejuaraan = $this->bm->select_all('ms_juara');
		$kategori = $this->bm->select_where('ms_kategori_juara', 'id_kegiatan', $kegiatan->id)->result();
		$array_kategori[''] = 'Pilih Kategori';
		foreach ($kategori as $key => $value) {
			$array_kategori[$value->id_kategori] = $value->nmkategori;
		}
		...
		$data['id_jenjang'] = $kdjenjang;
		$data['kegiatan_object'] = $kegiatan;
		$data['array_kategori'] = $array_kategori;
		$data['nmlomba'] = $this->bm->select_where('ms_lomba', 'id', $kegiatan->id_lomba)->row()->nmlomba;
		$data['nmjenjang'] = $nmjenjang;
		$data['jenjang'] = $array_jenjang;
		$data['kejuaraan'] = $kejuaraan;
		$data['title'] 	= "Input Nilai";
		$data['content'] = $this->load->view('admin/penilaian/vw_input_pos_sma',$data,true);
		$this->load->view('master_page',$data);
	}
