      ...
      <?php
       ...
       <div class="box box-default">
          <div class="box-header with-border" style="text-align: center !important;  ">
            <img src="<?php echo base_url(); ?>/assets/images/banner-penegak.png" width="100%" >
          ...
          <div class="row box-body">
            <form id="form-pendaftaran" method="POST" action="<?php echo base_url() ?>pendaftaran/penegak/post_sma">
              <div class="col-md-12">
                <table class="table table-striped table-responsive">
                  <tr>
                    <td width="15%">Jenjang</td>
                    <td>
                     Penegak/Pandega, Sekolah Menengah Atas
                    </th>
                  </tr>
                  <tr>
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-8">
                        <div class="table-responsive">
                          <div class="text-center"><h4><span class="glyphicon glyphicon-asterisk"></span><span class="font-bold"> Daftarkan Tim Campuran</span></h4></div>
                          <table class="table table-sm table-bordered">
                            ...
                            <?php $i=1; foreach ($lomba as $key => $value):?>
                                <?php if($value->id_lomba == 10) :?>
                                  <tr>
                                    <td><?php echo $i++; ?></td>
                                    <td><?php echo $value->nmlomba;?></td>
                                    <!-- <td><?php echo $value->jmlpeserta; ?></td> -->
                                    <?php if($pendaftar_lkbbt < $value->kuota): ?>
                                        <td class="text-right">Rp. <?php echo number_format(round($value->biaya),0, '' ,'.'); ?> x
                                          <input  type="hidden" name="biaya_regu_cp[]" class="form-control biaya_cp" value="<?php echo $value->biaya ?>">
                                        </td>
                                        <td width="30px"><input min="0" type="number" name="jumlah_regu_cp[]" class="form-control regu regu_cp" value="0"></td>
                                    <?php else : ?>
                                        <td colspan="2" class="text-center alert-danger"> Kuota Penuh
                                          <input min="0" type="number" name="jumlah_regu_cp[]" class="form-control regu regu_cp hide" value="0">
                                          <input  type="hidden" name="biaya_regu_cp[]" class="form-control biaya_cp" value="<?php echo $value->biaya ?>">
                                        </td>
                                    <?php endif ?>
                                  </tr>
                                ...
                                <?php $i=1; foreach ($lomba as $key => $value):?>
                                    <?php if($value->id_lomba != 10) :?>
                                      ...
                                      <?php else : ?>
                                          <td colspan="2" class="text-center alert-danger"> Kuota Penuh
                                            <input  type="hidden" name="biaya_regu_pa[]" class="form-control biaya_pa" value="<?php echo $value->biaya ?>">
                                            <input min="0" type="number" name="jumlah_regu_pa[]" class="form-control hide regu regu_pa" value="0">
                                          </td>
                                      <?php endif ?>
                                      </tr>
                                    <?php endif ?>
                                  ...
                                  <?php if($value->id_lomba != 10) :?>
                                    <tr>
                                      ...
                                      <?php else : ?>
                                          <td colspan="2" class="text-center alert-danger"> Kuota Penuh
                                            <input  type="hidden" name="biaya_regu_pi[]" class="form-control biaya_pi" value="<?php echo $value->biaya ?>">
                                            <input min="0" type="number" name="jumlah_regu_pi[]" class="regu form-control hide regu_pi" value="0">
                                          </td>
                                      <?php endif ?>
                                    </tr>
                                  <?php endif ?>
  ...
  function kabupatenChanges(){
   ...
   $('#kecamatan_select').on('change', function(e){
    ...
    function kecamatanChanges(){
      ...
      if(kecamatan){
        $('#warning-kecamatan').addClass('hide');
         //var kdProv = document.getElementById('provinsi').value;
       // alert(xx);
        var url = '<?php echo base_url()?>master/sekolah/get_api/'+kecamatan+'/'+3;
         //alert(v)
        $.ajax({
          //alert('test')
          url: url,
    ...
    function calculateRegu(){
      ...
      for(let i =0 ; i < regu_cp.length; i++){
        // console.log(regu_pa[i]);
        cp =  Number(regu_cp[i].value);
        cp_pay = Number(biaya_cp[i].value);
        total_regu += cp;
        total_biaya += Number(cp * cp_pay);
      }
      if(total_regu > 0){
